...
- 残り **deltaCAP の 30%**（18 ポイント）は**自動では配分しない**。CAP と 7 ステ合計の差（＝未割り振りポイント）として残し、**プレイヤーが居住区のステータス割り振り画面で割り振る**。
- レベルアップ直後は「合計 = CAP − 30% 増分」「各ステは 10% 以上」となり、プレイヤーが未割り振りポイントを配分すると「合計 = CAP」「各ステ 10〜30%」を満たせる。
...
1. **合計が CAP を超えない**
   - `STR + INT + VIT + WIS + DEX + AGI + LUK <= CAP` であればよい。一致は必須ではない（未割り振りポイントを残したまま保存できる）。
2. **割合下限（10%）**
...
> 10%×7 = 70% は「下限分」としてレベルアップ時に自動で各ステに振られ、残り 30% 分が自由割り振りになる。ステ割り振り保存時は合計が CAP を超えていなければよい。
...
- `VALIDATION` … 「合計が CAP を超えています」「各ステータスは CAP の 10〜30% の範囲で入力してください」など
...
3. DB から `CAP` を取得。
4. 入力された 7 ステの合計が CAP を超えていないか確認（合計 <= CAP）。
5. 合計が CAP 以下であり、各ステの値が
   - `>= floor(CAP * 0.10)`
   - `<= floor(CAP * 0.30)`
   を満たすか確認。
...
- 合計が CAP 未満のとき（レベルアップ直後など）は **未割り振りポイント（CAP − 7 ステ合計）** を表示し、プレイヤーがその分を配分して確定できるようにする。
...
5. 現在 `level` より大きければレベルアップ処理：
   ...
   - 7 ステは 2.6 に従い、各ステに `floor(deltaCAP×0.10)` のみ加算する（残り 30% は配分せず、CAP との差＝未割り振りポイントとしてプレイヤーが後で割り振る）。
