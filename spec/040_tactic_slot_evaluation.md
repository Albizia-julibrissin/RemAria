...
## 3. 主語の解決（subject → 対象のリスト／値）

条件は「主語で指定された対象」の状態を見る。主語によって **ユニットのリスト** を返すか、**サイクル／ターンという値** を条件の対象とするかが決まる。
...
### 3.2 サイクル・ターンを対象とする主語（主語で「値」を指定）

| subject（コード） | 意味 | 条件の対象 |
|-------------------|------|------------|
...
### 4.5 サイクル（主語 **cycle** または主語なし）

主語が **cycle** のとき、または主語を問わずコンテキストの cycle で判定する。主語を **cycle** にすると「サイクルが〇〇のとき」と明示できる（UI で「主語: サイクル」「条件: 偶数」など）。
...
主語が **turn** のとき、または主語を問わずコンテキストの turnIndexInCycle で判定する。主語を **turn** にすると「自分の行動順が〇～〇番目のとき」と明示できる（例: 先手だけ発動なら turnIndexMin=1, turnIndexMax=2）。
