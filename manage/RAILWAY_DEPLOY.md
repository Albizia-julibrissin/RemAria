...
- **Cursor に依頼する場合（推奨）**: **manage/production.env** に本番の `DATABASE_URL` を用意しておき、[PRODUCTION_RELEASE_GUIDE.md の「Cursor に本番リリースの各手順を依頼する」](./PRODUCTION_RELEASE_GUIDE.md#9-cursor-に本番リリースの各手順を依頼する標準のやり方) の手順で「本番にマイグレーション実行して」などと依頼する。.env を書き換えずに済む。
- 自分で実行する場合:
  ...
  3. ターミナルで実行: `npx prisma migrate deploy`、必要なら `npx prisma db seed`。
...
- **Railway の Volume バックアップ**: Postgres サービス → **Settings → Backups** で手動バックアップ作成やスケジュール（Daily/Weekly/Monthly）ができる。**復元**は同じタブで日時を選んで Restore → ステージングを Deploy する。DB に手を入れるリリース前の「念のため」に手動で 1 つ取っておくと安心。詳細は [PRODUCTION_RELEASE_GUIDE.md §6](./PRODUCTION_RELEASE_GUIDE.md#6-db-に関わるときの念のためバックアップ)。
- **手動で .dump を残す**: **Variables** の `DATABASE_URL` を一時的に本番にし、`npm run db:backup` で `manage/backups/` に .dump を出力する方法もある。**Cursor に依頼する場合**は、manage/production.env を読み込んだうえで `npm run db:backup` を実行してもらう（[PRODUCTION_RELEASE_GUIDE.md §9](./PRODUCTION_RELEASE_GUIDE.md#9-cursor-に本番リリースの各手順を依頼する標準のやり方)）。ローカルに残るので、Railway とは別の復元手段として使える。
