...
- **前提**:
  - 対象は **管理用アカウント**（環境変数 `ADMIN_EMAIL` で指定されたユーザー、未設定時は `test1@example.com`）でログインしている場合のみ。それ以外は API が null を返すか redirect する。
...
いずれも `src/server/actions/admin.ts`。**管理用アカウント**でログインしている場合のみ実行可能。
