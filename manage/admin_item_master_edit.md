...
Item マスタの code / name / category / skillId / consumableEffect / maxCarryPerExpedition を編集する管理機能の手順と仕様。
...
- **目的**: 運用中にアイテムの表示名・カテゴリ・スキル分析書の紐づけ・消耗品効果・探索持ち込み上限を、コードを触らずに変更できるようにする。
...
| **操作** | 一覧で「新規作成」→ 入力 → 「作成」。または一覧で「編集」→ 編集画面で変更 → 「保存」。 |
