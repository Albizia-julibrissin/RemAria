...
Item マスタの code / name / category / skillId / consumableEffect / maxCarryPerExpedition / **maxOwnedPerUser**（ユーザー別所持数上限）を編集する管理機能の手順と仕様。加えて、**装備型（EquipmentType）の名前**も同一画面で編集できる。
...
- **目的**: 運用中にアイテムの表示名・カテゴリ・スキル分析書の紐づけ・消耗品効果・探索持ち込み上限・**ユーザー別所持数上限（maxOwnedPerUser）**を、コードを触らずに変更できるようにする。あわせて、クラフトで製造する**装備型の表示名（name）**も編集できる（どの装備をレシピで作るかはクラフトレシピ編集で設定）。
...
| 関数名 | 役割 |
|--------|------|
| `getAdminItemList()` | 全 Item 一覧（id, code, name, category, skillId, skillName, consumableEffect, maxCarryPerExpedition, maxOwnedPerUser）。 |
| `getAdminItem(itemId)` | 1件取得（編集フォーム用）。 |
| `getAdminSkillsForItem()` | スキル一覧（skill_book の skillId 選択用）。 |
| `updateAdminItem(itemId, input)` | 1件更新。input: code, name, category, skillId, consumableEffectJson（文字列）, maxCarryPerExpedition, maxOwnedPerUser。 |
...
- **Item**: id, code（ユニーク）, name, category, skillId（skill_book 時）, consumableEffect（Json）, maxCarryPerExpedition（Int?）, **maxOwnedPerUser（Int?）**（ユーザー別所持数上限。スタック可能アイテム用。null は上限なし）。schema は `prisma/schema.prisma`。
