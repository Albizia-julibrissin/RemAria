// 敵マスタ一覧（管理者用・テストユーザー1のみ）

import Link from "next/link";
import { redirect } from "next/navigation";
import { getAdminEnemyList } from "@/server/actions/admin";
...
export default async function AdminEnemiesPage() {
  ...
  const list = await getAdminEnemyList();
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-text-muted">
            Enemy（code / name / 基礎ステ・配置・作戦スロット）。spec/050。
          </p>
        </div>
        <Link
          href="/dashboard/admin/enemies/new"
        ...
        <table className="w-full min-w-[560px] text-sm border-collapse border border-base-border">
          <thead>
            <tr className="bg-base-elevated">
              ...
              <th className="border border-base-border px-2 py-1.5 text-right text-text-muted font-medium">CAP</th>
          ...
          <tbody>
            {list.map((row) => (
              <tr key={row.id} className="text-text-primary">
                ...
                <td className="border border-base-border px-2 py-1.5 text-right text-text-muted tabular-nums">{row.CAP}</td>
