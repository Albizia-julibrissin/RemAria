// スキル 1件編集（管理者用・テストユーザー1のみ）

import Link from "next/link";
import { redirect, notFound } from "next/navigation";
import { getAdminSkillEditData } from "@/server/actions/admin";
import { isTestUser1 } from "@/server/lib/admin";
import { AdminSkillEditForm } from "./admin-skill-edit-form";
...
export default async function AdminSkillEditPage({
  ...
  const data = await getAdminSkillEditData(id);
  ...
  return (
    <main className="min-h-screen bg-base p-8">
      ...
      <p className="mt-2 text-sm text-text-muted">{data.skill.name}</p>

      <AdminSkillEditForm data={data} />
    </main>
  );
}
