...
import { getAdminRelicType } from "@/server/actions/admin";
import { isTestUser1 } from "@/server/lib/admin";
import { AdminRelicTypeEditForm } from "./admin-relic-type-edit-form";

export default async function AdminRelicTypeEditPage({ params }: { params: Promise<{ id: string }> }) {
  if (!(await isTestUser1())) redirect("/dashboard");
  const { id } = await params;
  const row = await getAdminRelicType(id);
  if (!row) notFound();
  return (
    <main className="min-h-screen bg-base p-8">
      ...
      <AdminRelicTypeEditForm relicType={row} />
    </main>
  );
}
