...
import type {
  AdminExplorationEventDetail,
  AdminSkillEventStatOptionRow,
  UpdateAdminExplorationEventInput,
} from "@/server/actions/admin";
import { updateAdminExplorationEvent } from "@/server/actions/admin";

const STAT_KEYS = ["STR", "INT", "VIT", "WIS", "DEX", "AGI", "LUK"] as const;
const DEFAULT_SUCCESS = "うまくいった。";
const DEFAULT_FAIL = "うまくいかなかった。";

function mergeStatOptions(existing: AdminSkillEventStatOptionRow[]): {
  statKey: string;
  sortOrder: number;
  ...
  const map = new Map(existing.map((o) => [o.statKey, o]));
  return STAT_KEYS.map((statKey, i) => {
    const o = map.get(statKey);
    return o
      ? { ...o, statKey }
      : {
          statKey,
...
type Props = { event: AdminExplorationEventDetail };

export function AdminSkillEventEditForm({ event }: Props) {
  ...
  const [occurrenceMessage, setOccurrenceMessage] = useState(event.occurrenceMessage);
  const [statOptions, setStatOptions] = useState(() => mergeStatOptions(event.statOptions));

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const input: UpdateAdminExplorationEventInput = {
      code: code.trim(),
      name: name.trim(),
    ...
    startTransition(async () => {
      const result = await updateAdminExplorationEvent(event.id, input);
      setMessage(
