// 研究グループ 1件編集（管理者用・テストユーザー1のみ）

import Link from "next/link";
import { redirect, notFound } from "next/navigation";
import { getAdminResearchGroupEditData } from "@/server/actions/admin";
import { isTestUser1 } from "@/server/lib/admin";
import { AdminResearchGroupEditForm } from "./admin-research-group-edit-form";

export default async function AdminResearchGroupEditPage({
  params,
}: {
  ...
  const data = await getAdminResearchGroupEditData(id);
  ...
  return (
    <main className="min-h-screen bg-base p-8">
      ...
      <AdminResearchGroupEditForm data={data} />
    </main>
  );
}
