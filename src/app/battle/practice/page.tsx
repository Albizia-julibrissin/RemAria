// spec/020_test_battle.md - 練習戦闘画面。プリセットを選んで戦闘実行。

import { redirect } from "next/navigation";
import { getProtagonist } from "@/server/actions/protagonist";
import { getPartyPresets } from "@/server/actions/tactics";
import { BattlePracticeClient } from "./battle-practice-client";
import Link from "next/link";

export default async function BattlePracticePage() {
  ...
  if (presets.length === 0) {
    return (
      <main className="min-h-screen bg-base p-8">
        <h1 className="text-2xl font-bold text-text-primary">練習戦闘</h1>
  ...
  return (
    <main className="min-h-screen bg-base p-8">
      <h1 className="text-2xl font-bold text-text-primary">練習戦闘</h1>
      <p className="mt-2 text-text-muted">
        使用するプリセットを選んで戦闘開始してください。主人公 vs スライム3体（1v3）。敵は中列・後列に配置。
      </p>
      <div className="mt-6">
        <BattlePracticeClient presets={presets} />
      </div>
      <p className="mt-6">
        <Link href="/dashboard" className="text-sm text-brass hover:text-brass-hover">
          ← ダッシュボードへ
