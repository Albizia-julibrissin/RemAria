...
import { runBattle, type RunBattleResult } from "@/server/actions/battle";
import { BattleFullView } from "./battle-full-view";
import type { PartyPresetWithCharacters } from "@/server/actions/tactics";

interface BattlePracticeClientProps {
  presets: PartyPresetWithCharacters[];
}

export function BattlePracticeClient({ presets }: BattlePracticeClientProps) {
  const [selectedPresetId, setSelectedPresetId] = useState(presets[0]?.id ?? "");
  const [result, setResult] = useState<RunBattleResult | null>(null);
  ...
  async function handleStart() {
    ...
    setResult(null);
    try {
      const res = await runBattle(selectedPresetId);
      setResult(res);
    } finally {
      setLoading(false);
    }
