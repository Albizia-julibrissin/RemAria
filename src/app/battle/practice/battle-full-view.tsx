...
import { DEFAULT_ENEMY_NAME, DEFAULT_ENEMY_ICON_FILENAME } from "@/lib/battle/default-enemy";
import { AllyGridRow, EnemyGridRow } from "./battle-grid-view";
import { EntryLines } from "./battle-log-view";

type LogEntry = RunBattleSuccess["log"][number];
...
/** 表示順：1行目＝味方1/敵陣地1行目、2行目＝味方2/敵陣地2行目、3行目＝味方3/敵陣地3行目。敵のHPは「その行にいる敵」と一致させる（enemyIndex は位置で決める）。 */
const DISPLAY_LINES: { allyRow: 1 | 2 | 3; partyIndex: number; enemyRow: 1 | 2 | 3 }[] = [
  { allyRow: 1, partyIndex: 0, enemyRow: 1 },
  { allyRow: 2, partyIndex: 1, enemyRow: 2 },
  { allyRow: 3, partyIndex: 2, enemyRow: 3 },
];

/** 各行にどの敵（index）がいるか。enemyPositions[i].row === row となる i を返す。 */
function getEnemyIndexOnRow(enemyPositions: BattlePosition[], row: 1 | 2 | 3): number | null {
  const i = enemyPositions.findIndex((p) => p.row === row);
  return i >= 0 ? i : null;
}
...
function TurnBlock({
  ...
  entries: LogEntry[];
  protagonistPosition: RunBattleSuccess["protagonistPosition"];
  protagonistIconFilename: RunBattleSuccess["protagonistIconFilename"];
  partyIconFilenames: (string | null)[];
  /** このターン終了時点の味方の陣地位置 */
  partyPositions: BattlePosition[];
  /** このターン終了時点の敵の陣地位置 */
  enemyPositions: BattlePosition[];
  summary: RunBattleSuccess["summary"];
          ...
          return (
            ...
            >
              ...
              <div className="shrink-0 w-28 max-sm:w-20">
                {enemyIndex != null ? (
                  <CompactUnit
                    name={enemyDisplayNames[enemyIndex] ?? `${DEFAULT_ENEMY_NAME}${enemyIndex + 1}`}
    ...
    <div className="mt-6 space-y-6">
      ...
      <div className="space-y-6">
        {groupLogByTurn(data.log).map(({ cycle, turn, entries }) => {
          ...
          const enemyNames = data.enemyDisplayNames ?? [1, 2, 3].map((i) => `${DEFAULT_ENEMY_NAME}${i}`);
          ...
          return (
            <TurnBlock
              ...
              defaultEnemyIconFilename={DEFAULT_ENEMY_ICON_FILENAME}
            />
          );
        })}
      </div>
